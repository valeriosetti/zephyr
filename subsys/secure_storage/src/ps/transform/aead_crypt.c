...
BUILD_ASSERT(SECURE_STORAGE_PS_TRANSFORM_AEAD_TAG_SIZE == 16);
		...
		const uint8_t *input, size_t output_size, uint8_t *output, size_t *output_len)
{
	...
	uint8_t key[CONFIG_SECURE_STORAGE_PS_TRANSFORM_AEAD_KEY_SIZE];
	...
	secure_storage_ps_transform_aead_get_scheme(&key_type, &alg);
	...
	ret = secure_storage_ps_transform_aead_get_key(uid, key);
	if (ret != PSA_SUCCESS) {
		goto exit;
	}

	ret = aead_crypt(&key_attributes, key, sizeof(key), alg, nonce,
			 CONFIG_SECURE_STORAGE_PS_TRANSFORM_AEAD_NONCE_SIZE, add_data,
